...
if($_POST['submit'])
{
    ...
    // start new agent_queue
    $queue_manager->delete_all_agent_queue($id); 
    
    $admin_data = $queue_manager->select_admin_licence($id);
    $licence_key = $admin_data[0]['licence_key'];
    $queue_name = $admin_data[0]['name'];


    $conn4 = $db->connect4();
    $queue_manager->conn4 = $conn4;
    $client_data_by_licence = $queue_manager->select_client_by_licence($licence_key);
    $client_id = $client_data_by_licence[0]['id'];
    $client_name = $client_data_by_licence[0]['name'];
...
//loop
    ...
    foreach ($_POST['s_agent'] as $a)
    {
        $agent_data_by_id = $queue_manager->select_agent_by_id($a);

        $agent_name = $agent_data_by_id[0]['name'];

        $queue_manager->set_agent_queue($a, $agent_name, $id, $queue_name, $client_id, $client_name, $licence_key);
    }
...
<select class="select select-m medium-input" title="Select Queue" multiple="multiple" name="s_agent[]">
    <?php $get_s_agent=$queue_manager->select_agents_by_queue_id($id); 
    foreach ($get_agents as $ag)
    {
       $selected="";
       foreach($get_s_agent as $sa) { if($sa['agent_id'] == $ag['id']) $selected="selected"; }
